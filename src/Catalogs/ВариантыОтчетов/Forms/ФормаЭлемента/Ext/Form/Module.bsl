...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПредопределенный = Не Объект.Пользовательский
		И (Объект.ТипОтчета = Перечисления.ТипыОтчетов.Внутренний
			Или Объект.ТипОтчета = Перечисления.ТипыОтчетов.Расширение)
		И ЗначениеЗаполнено(Объект.ПредопределенныйВариант);
		
	Если ЭтоПредопределенный Тогда	
		ПредопределенныйВариант = Объект.ПредопределенныйВариант.ПолучитьОбъект();
		ЛокализацияСервер.ПриЧтенииПредставленийНаСервере(ПредопределенныйВариант);
		
		ПредопределеннаяВидимостьПоУмолчанию = ПредопределенныйВариант.ВидимостьПоУмолчанию;
		ПредопределенноеОписание = ПредопределенныйВариант.Описание;
		ЗаполнитьИзПредопределенного();
	КонецЕсли;	
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоПредопределенный Тогда
		ТекущийОбъект.ВидимостьПоУмолчаниюПереопределена = 
			Объект.ВидимостьПоУмолчанию <> ПредопределеннаяВидимостьПоУмолчанию;
		Если Не ПустаяСтрока(Объект.Описание) И НРег(СокрЛП(Объект.Описание)) = НРег(СокрЛП(ПредопределенноеОписание)) Тогда
...
&НаСервере
Процедура ЗаполнитьИзПредопределенного()
	...
	Если ПустаяСтрока(Объект.Описание) Тогда
		Объект.Описание = ПредопределенноеОписание;
	КонецЕсли;
КонецПроцедуры
