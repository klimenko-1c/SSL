...
Процедура ВыводитьЗаголовокПриИзменении(Элемент)
	...
	ЭлементНастройки = ПараметрыВывода.Элементы.Найти("TITLE");
...
Процедура ВыводитьПараметрыИОтборыПриИзменении(Элемент)
	...
	ЭлементНастройки = ПараметрыВывода.Элементы.Найти("DATAPARAMETERSOUTPUT");
...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда 
			...
			Строка.Заголовок = ЭлементНастройки.Заголовок;
			Если Строка.Расположение <> РасположениеПоляКомпоновкиДанных.Авто Тогда 
				Строка.Заголовок = Строка.Заголовок + " (" + Строка.Расположение + ")";
			КонецЕсли;
...
Процедура ВыбранныеПоляПередГруппировкойПолей(СвойстваГруппы, ПараметрыГруппировки) Экспорт
	...
	Если ЭлементНастройкиПриемник.Расположение <> РасположениеПоляКомпоновкиДанных.Авто Тогда 
		СтрокаПриемник.Заголовок = СтрокаПриемник.Заголовок + " (" + Строка(ЭлементНастройкиПриемник.Расположение) + ")";
	КонецЕсли;
...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка)
	...
	ТекущееЗначение = ?(Строка.ЭтоПараметр, Строка.Значение, Строка.ПравоеЗначение);
	...
	ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(ТекущиеНастройки, ПользовательскиеНастройки, ЭлементНастройки);
	...
	Условие = ОтчетыКлиентСервер.УсловиеЭлементаНастройки(ЭлементНастройки, ОписаниеЭлементаНастройки);
	ВыборГруппИЭлементов = ОтчетыКлиент.ЗначениеТипаИспользованиеГруппИЭлементов(Строка.ВыборГруппИЭлементов, Условие);
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка = Оформление.ПолучитьЭлементы().Добавить();
	Строка.Использование = (Объект.Значение = ТипВыводаТекстаКомпоновкиДанных.Авто);
...
Процедура СинхронизироватьПредопределенныеПараметрыВывода(Использование, ЭлементНастройки)
	...
	Если ЭлементНастройки.Параметр = Новый ПараметрКомпоновкиДанных("Заголовок") Тогда 
		СвязанныйЭлементНастройки = ПараметрыВывода.Элементы.Найти("TITLEOUTPUT");
	...
	ИначеЕсли ЭлементНастройки.Параметр = Новый ПараметрКомпоновкиДанных("ВыводитьПараметрыДанных") Тогда 
		СвязанныйЭлементНастройки = ПараметрыВывода.Элементы.Найти("FILTEROUTPUT");
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	ИначеЕсли ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда 
		Объекты = Элемент.Родитель.ДоступныеОбъекты.Элементы;
...
Процедура ОбновитьФорму(ПараметрыОбновления = Неопределено)
	...
	Если РасширенныйРежим = 0 Тогда 
		ОтчетыСервер.ОбновитьЭлементыФормыНастроек(ЭтотОбъект, Элементы.Основное);
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	НастройкиЗагружены = ОтчетыКлиентСервер.ЗагрузитьНастройки(
		Отчет.КомпоновщикНастроек, ДоступныеНастройки.Настройки, ДоступныеНастройки.ПользовательскиеНастройки);
...
Процедура ИнициализироватьРеквизитыПредопределенныхПараметровВывода()
	Объект = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("TITLE");
	ВыводитьЗаголовок = Объект.Использование;
	
	Объект = Отчет.КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("DATAPARAMETERSOUTPUT");
	ВыводитьПараметрыИОтборы = (Объект.Значение = ТипВыводаТекстаКомпоновкиДанных.Авто);
КонецПроцедуры
