...
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, Знач АдресаДляПроверки)
	...
	ИначеЕсли РезультатПроверки.КодСостояния = 400 Тогда
		...
		Результат.ПодробноеПредставлениеОшибки = НСтр("ru='Не указан регион адреса'");
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Не указан регион адреса'");
		Результат.Отказ = Истина;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес)
					...
					Если СтрСравнить(НомерДомаАдрес, Дом) = 0 Тогда
						...
						Если СтрСравнить(ТипДомаАдрес, НазваниеВладения) = 0 Тогда
							...
							Иначе
								...
								НовыйВариант.Дом = НРег(НазваниеВладения) + " " + Дом + ?(ПустаяСтрока(ТекстПроСтроения), "", ", " + ТекстПроСтроения);
	...
	Если НЕ ДомаКорректный Тогда
		...
		Если ПохожиеВарианты.Количество() = 1 Тогда
			Подсказка = ", " + НСтр("ru = 'но есть'") + " " + """" + ПохожиеВарианты[0].Дом + """.";
		ИначеЕсли ПохожиеВарианты.Количество() > 1 Тогда
			ПервоеЗначение = ПохожиеВарианты[0].Дом;
			Если ПохожиеВарианты.Количество() = 2 Тогда
				Подсказка = НСтр("ru = 'но есть ""%1"" или ""%2"".'");
				ВтороеЗначение = ПохожиеВарианты[1].Дом;
			Иначе
				Подсказка = НСтр("ru = 'но есть ""%1"" и другие варианты(%2).'");
				ВтороеЗначение = Строка(ПохожиеВарианты.Количество() - 1);
			КонецЕсли;
		...
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, ПолеСОшибкой,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" отсутствует в ФИАС%2'"), ПредставлениеДома, Подсказка));
		Возврат Результат;
