...
// операций, в налоговых вычетах и т.п. В них недопустимо сокращение количества полей на форме, перекомпоновка).
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ЗначениеЗаполнено(ПечатнаяФорма.ОфисныеДокументы) Тогда
			Для Каждого ОфисныйДокумент Из ПечатнаяФорма.ОфисныеДокументы Цикл
				...
				Файл.ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ОфисныйДокумент.Значение + ".docx");
...
Процедура УбратьПодписьИПечать(ТабличныйДокумент) Экспорт
...
Функция ПрефиксыИменОбластейСПодписьюИПечатью()
