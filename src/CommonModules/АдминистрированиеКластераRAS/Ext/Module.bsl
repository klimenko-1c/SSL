		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1
				           ...
				           |следует его запустить, например, так:
				           |
				           |""%3ras.exe"" cluster --port=%4 %5:%6
				           |
				           |В противном случае следует проверить параметры подключения и сетевого экрана.'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),
				ИмяКомпьютера(),
				КаталогПрограммы(),
				Формат(ПараметрыАдминистрированияКластера.ПортСервераАдминистрирования, "ЧГ="),
				ПараметрыАдминистрированияКластера.АдресАгентаСервера,
				Формат(ПараметрыАдминистрированияКластера.ПортАгентаСервера, "ЧГ="));
		КонецПопытки;
