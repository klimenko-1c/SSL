...
Функция ЗагрузитьНастройки(КомпоновщикНастроекКД, НастройкиКД, ПользовательскиеНастройкиКД = Неопределено) Экспорт
	НастройкиЗагружены = (ТипЗнч(НастройкиКД) = Тип("НастройкиКомпоновкиДанных")
		И НастройкиКД <> КомпоновщикНастроекКД.Настройки);
	...
	Если НастройкиЗагружены Тогда
		...
		ДоступныеЗначения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			КомпоновщикНастроекКД.Настройки.ДополнительныеСвойства, "ДоступныеЗначения");
		
		Если ДоступныеЗначения <> Неопределено Тогда 
			НастройкиКД.ДополнительныеСвойства.Вставить("ДоступныеЗначения", ДоступныеЗначения);
		КонецЕсли;
...
Функция ЗначенияЭлементовНастроек(Настройки, ПользовательскиеНастройки, Поле)
	...
	Для Каждого Элемент Из ЭлементыНастроек Цикл 
		...
		Если ЭлементАнализируемый.Использование
			...
			Если ТипЗнч(ЭлементАнализируемый.Значение) = Тип("СписокЗначений") Тогда 
				ДополнитьСписок(Значения, ЭлементАнализируемый.Значение, Ложь, Истина);
			Иначе
				Значения.Добавить(ЭлементАнализируемый.Значение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ЭлементыНастроек = Настройки.Отбор.Элементы;
	Для Каждого Элемент Из ЭлементыНастроек Цикл 
		ЭлементПользовательский = ПользовательскиеНастройки.Найти(Элемент.ИдентификаторПользовательскойНастройки);
		ЭлементАнализируемый = ?(ЭлементПользовательский = Неопределено, Элемент, ЭлементПользовательский);
		
		Если ЭлементАнализируемый.Использование И Элемент.ЛевоеЗначение = Поле Тогда 
			Если ТипЗнч(ЭлементАнализируемый.ПравоеЗначение) = Тип("СписокЗначений") Тогда 
				ДополнитьСписок(Значения, ЭлементАнализируемый.ПравоеЗначение, Ложь, Истина);
			Иначе
				Значения.Добавить(ЭлементАнализируемый.ПравоеЗначение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПараметрыВыбора(Настройки, ПользовательскиеНастройки, ЭлементНастройки) Экспорт 
	...
	Для Каждого Параметр Из Параметры Цикл 
		...
		Значения = ЗначенияЭлементовНастроек(Настройки, ПользовательскиеНастройки, Параметр.Поле);
		Если Значения.Количество() > 0 Тогда 
			ПараметрыВыбора.Добавить(Новый ПараметрВыбора(Параметр.Имя, Значения));
		КонецЕсли;
	КонецЦикла;
