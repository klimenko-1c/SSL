...
//   * ПротоколПочты - Строка - если указано значение "IMAP", то письмо будет передано по протоколу IMAP, если по указано значение "Все", то по 
//                              протоколу SMTP и по протоколу IMAP, если ничего не указано то по протоколу SMTP.
//                              Параметр имеет смысл, только наличии действующего соединения, указанного в параметр Соединение.
//                              В противном случае протокол будет определен автоматически при установке соединения.
//   * ИдентификаторСообщения - Строка - (возвращаемый параметр) идентификатор отправленного почтового сообщения на SMTP
//                                       сервере;
//   * ИдентификаторСообщенияОтправкаIMAP - Строка - (возвращаемый параметр) идентификатор отправленного почтового сообщения на IMAP
//                                         сервере;
