...
Функция БезопаснаяСтрокаКоманды(КомандаЗапуска) Экспорт
	...
	ИначеЕсли ТипЗнч(КомандаЗапуска) = Тип("Массив") Тогда
		...
		Иначе
			ВызватьИсключение
				НСтр("ru = 'Ожидалось, что в первый элемент КомандаЗапуска будет командой или путем к исполняемому файлу.'");
		КонецЕсли;
...
Функция НовыйФайлЗапускаКомандыWindows(СтрокаКоманды, ТекущийКаталог, ДождатьсяЗавершения, КодировкаИсполнения) Экспорт
	...
	Если ВРег(КодировкаИсполнения) = "OEM" Тогда
		ТекстовыйДокумент.ДобавитьСтроку("chcp 437");
	КонецЕсли;
	
	Если ВРег(КодировкаИсполнения) = "UTF8" Тогда
		ТекстовыйДокумент.ДобавитьСтроку("chcp 65001");
	КонецЕсли;
