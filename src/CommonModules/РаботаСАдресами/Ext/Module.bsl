...
//
// Параметры:
//   Адреса                 - Массив - адреса во внутреннем формате JSON или в XML, соответствующем XDTO пакету Адрес
//                                     или XDTO объектов, соответствующий XDTO пакету Адрес.
...
//
// Возвращаемое значение:
//   Массив - Содержит массив структур, содержимое структуры см. описание в функции СведенияОбАдресе.
//
...
//       * БезПредставлений - Булево - Если Истина, то поле Представление будет отсутствовать. По умолчанию, Ложь.
//       * КодыКЛАДР        - Булево - Если Истина, то возвращается структура КодыКЛАДР. По умолчанию, Ложь.
//       * КодыАдреса       - Булево - Если Истина, то возвращается коды адреса. По умолчанию, Истина.
//       * ПолноеНаименованиеСокращений - Булево - Если Истина, то возвращается полное наименование адресных объектов.
//       * НаименованиеВключаетСокращение - Булево - Если Истина, то поля содержат сокращениям в наименованиях адресных объектов.
...
Функция СведенияОбАдресеВВидеСтруктуры(Знач АдресСтрокой, ДополнительныеПараметры)
	...
	Параметры = ИнициализироватьСвойстваПоПараметрам(ДополнительныеПараметры, Результат);
	...
	Если Не ЗначениеЗаполнено(Адрес.CountryCode) Тогда
	...
	Для каждого ЧастьАдреса Из СоответствиеУровней Цикл
		...
		Результат[ЧастьАдреса.Значение.Имя + "Сокращение"] = Сокращение;
...
Функция ИнициализироватьСвойстваПоПараметрам(Знач ДополнительныеПараметры, Знач Результат)
