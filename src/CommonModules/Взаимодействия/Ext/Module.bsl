	...
	Если Соединение = Неопределено Тогда
		...
		Попытка
			...
			Если ПротоколПодключения = ПротоколИнтернетПочты.IMAP Тогда
				...
				Если Не (Объект.УдалятьПослеОтправки Или
					УправлениеЭлектроннойПочтой.СерверИсходящейПочтыНеТребуетДополнительнойОтправкиПоIMAP(Профиль.АдресСервераSMTP)) Тогда
