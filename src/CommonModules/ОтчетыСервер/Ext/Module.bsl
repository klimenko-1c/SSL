...
//
// Параметры:
//   Отчет       - ОтчетОбъект, ВнешнийОтчетОбъект - отчет, для которого необходимо подключить схему.
...
//   Схема       - СхемаКомпоновкиДанных - схема, которую необходимо подключить.
...
Процедура ОбновитьЭлементыФормыНастроек(Форма, УзелИерархииЭлементов) Экспорт 
	...
	СвойстваЭлементов = СвойстваЭлементовФормыНастроек(
		Форма.ТипФормыОтчета, Форма.Отчет.КомпоновщикНастроек, Форма.НастройкиОтчета);
...
Функция ДоступныеНастройки(ПараметрыЗагрузки, НастройкиОтчета) Экспорт 
	...
	Возврат Новый Структура("Настройки, ПользовательскиеНастройки", Настройки, ПользовательскиеНастройки);
КонецФункции
...
Процедура ИнициализироватьПредопределенныеПараметрыВывода(Контекст, Настройки) Экспорт 
	...
	ЗаполнитьЗначенияСвойств(СвязанныйОбъект, Объект, "Использование, Значение");
	СвязанныйОбъект.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	СвязанныйОбъект.ИдентификаторПользовательскойНастройки = "";
КонецПроцедуры
...
Процедура УстановитьЗаголовокОтчетаПоУмолчанию(Заголовок, Контекст)
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст, "Внешний", Ложь) Тогда 
	...
	МетаданныеОтчета = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(ИдентификаторОтчета);
	Если МетаданныеОтчета <> Неопределено Тогда 
		Заголовок.Значение = МетаданныеОтчета.Синоним;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучитьСведенияОЭлементахСвойстваНастроек(Настройки, Свойство, ИдентификаторСвойства, Сведения)
	...
	Для Каждого Элемент Из Свойство.Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда 
			Описание = Свойство.ДоступныеПоляОтбора.НайтиПоле(Элемент.ЛевоеЗначение);
		ИначеЕсли ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда 
			Описание = Свойство.ДоступныеПараметры.НайтиПараметр(Элемент.Параметр);
		КонецЕсли;
